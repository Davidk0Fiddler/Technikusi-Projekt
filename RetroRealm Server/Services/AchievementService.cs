...
namespace RetroRealm_Server.Services
{
    public class AchievementService: IAchievementsService
    {
        ...
        public async Task<Result<ReadAchievementDTO>> GetAchievementAsync(int id)
        {
            ...
            await _logService.CreateLogAsync(LogType.Get.ToString(), null, $"Achievement ({achievement.Name} has been requested!", DateTime.Now, null);
        ...
        public async Task<Result<Achievement>> CreateAchievementAsync(CreateAchievementDTO newAchievement)
        {
            ...
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Database error while creating an achievement! {achievement.Name}", DateTime.Now, null);
                return Result<Achievement>.Fail("Database Error");
            }
            catch (Exception ex)
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Error while creating an achievement! {achievement.Name}", DateTime.Now, null);
        ...
        public async Task<Result<UpdateAchievementDTO>> UpdateAchievementAsync(int id, UpdateAchievementDTO updatedAchievement)
        {
            ...
            currentAchievement.Name = currentAchievement.Name;
            currentAchievement.Description = currentAchievement.Description;
            ...
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Database error during updating achievement! {currentAchievement.Name}", DateTime.Now, null);
                return Result<UpdateAchievementDTO>.Fail("Database error");
            }
            catch (Exception ex)
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Error during updating achievement! {currentAchievement.Name}", DateTime.Now, null);
        ...
        public async Task<Result<int>> DeleteAchievementByIdAsync(int id)
        {
            ...
            {
                await _context.SaveChangesAsync();
                await _logService.CreateLogAsync(LogType.Delete.ToString(), null, $"Achievements ({DeletingItem.Name}) has been deleted!", DateTime.Now, null);
                return Result<int>.Ok(id);
            }
            catch (DbUpdateException ex)
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Database error during deleting achievements! {DeletingItem.Name}", DateTime.Now, null);
                return Result<int>.Fail("Database error");
            }
            catch (Exception ex)
            {
                await _logService.CreateLogAsync(LogType.Error.ToString(), ex.Message, $"Error during deleting achievements! {DeletingItem.Name}", DateTime.Now, null);
